...
const searchMemosMemory = async (payload: MemosSearchPayload): Promise<MemosSearchResult> => {
...
const addMemosMessage = async (payload: MemosAddPayload): Promise<MemosAddResult> => {
  const settings = normalizeSettings(payload.settings);
  if (!settings.memos.enabled) {
    return {
      ok: false,
...
const testMemosConnection = async (settings: AppSettings): Promise<ConnectionTestResult> => {
...
const registerIpcHandlers = () => {
  ...
  ipcMain.handle(
    ...
    async (_, settings: AppSettings): Promise<ConnectionTestResult> => testMemosConnection(settings)
  );

  ipcMain.handle(
    "memos:searchMemory",
    async (_, payload: MemosSearchPayload): Promise<MemosSearchResult> => searchMemosMemory(payload)
  );

  ipcMain.handle(
    "memos:addMessage",
    async (_, payload: MemosAddPayload): Promise<MemosAddResult> => addMemosMessage(payload)
  );
