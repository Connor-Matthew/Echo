...
const CodeBlock = ({ code, language }: { code: string; language?: string }) => {
  ...
  return (
    <div className="mb-2 overflow-hidden rounded-lg border border-[#d6e1ec] bg-[#f6f9fd] last:mb-0">
      <div className="flex items-center justify-between border-b border-[#dfe7f1] bg-[#eef4fb] px-2.5 py-1.5">
        <span className="text-[11px] font-medium uppercase tracking-wide text-[#5a7289]">
          {language || "code"}
        </span>
        <Button
          type="button"
          variant="ghost"
          size="sm"
          className="h-6 px-2 text-[11px] text-[#4f6478]"
          onClick={() => {
            void copyCode();
          }}
        >
      ...
      <pre className="overflow-x-auto p-3">
        <code className="font-mono text-[13px] leading-6 text-[#203347]">{code}</code>
      </pre>
    </div>
  );
};
...
const markdownComponents: Components = {
  ...
  blockquote: ({ children }) => (
    <blockquote className="mb-2 border-l-2 border-[#b8cce1] pl-3 text-[#324a63]">
      {children}
    </blockquote>
  ),
  a: ({ children, href }) => (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="text-[#346b9c] underline decoration-[#8fb4d2] underline-offset-2"
    >
      {children}
    </a>
  ),
  ...
  code: ({ children, className }) => {
    ...
    if (isBlock) {
      return <code className="font-mono text-[13px] leading-6 text-[#203347]">{children}</code>;
    }
    return (
      <code className="rounded bg-[#e4edf7] px-1 py-[1px] font-mono text-[13px] text-[#2e4a66]">
        {children}
      </code>
    );
  },
  ...
  thead: ({ children }) => <thead className="bg-[#e9f1f8]">{children}</thead>,
  tbody: ({ children }) => <tbody>{children}</tbody>,
  th: ({ children }) => (
    <th className="border border-[#d2deea] px-2 py-1.5 font-semibold text-[#304960]">{children}</th>
  ),
  td: ({ children }) => <td className="border border-[#d2deea] px-2 py-1.5">{children}</td>,
  hr: () => <hr className="my-3 border-[#d4e0ec]" />
};

const MarkdownContent = ({ content, isUser }: { content: string; isUser: boolean }) => (
  <div className={["text-[15px] leading-[1.65] break-words", isUser ? "text-[#18222c]" : "text-[#18222c]"].join(" ")}>
...
const MessageBubble = ({
  ...
  return (
    <div className={`group flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser ? (
        <div className="grid h-8 w-8 place-content-center rounded-xl border border-[#d8e4f1] bg-[#ebf1f8] text-[10px] font-semibold tracking-wide text-[#3e536a]">
          AI
        </div>
      ) : null}
      <div className={`flex max-w-[72%] flex-none flex-col ${isUser ? "items-end" : "items-start"}`}>
        {isUser && isEditing ? (
          <div
            className={[
              "rounded-[14px] border bg-[#eef3f8] px-3 py-2.5 transition-colors",
              isDragOverEdit ? "border-[#8bb0d4] bg-[#e7f0f9]" : "border-[#dce8f5]"
            ].join(" ")}
            onDragOver={(event) => {
          ...
          >
            ...
            <textarea
              ...
              className="min-h-[80px] w-full resize-y rounded-md border border-[#c8d7e7] bg-white p-2 text-[14px] leading-6 text-[#243447] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            {editAttachments.length ? (
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {editAttachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="rounded-xl border border-[#d8e1ea] bg-white px-2.5 py-2"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          {attachment.kind === "image" ? (
                            <ImageIcon className="h-3.5 w-3.5 text-[#5d6e81]" />
                          ) : (
                            <FileText className="h-3.5 w-3.5 text-[#5d6e81]" />
                          )}
                          <p className="truncate text-xs font-medium text-[#31465d]">{attachment.name}</p>
                        </div>
                        <p className="mt-0.5 text-[11px] text-[#6d7f93]">
                    ...
                    {attachment.error ? (
                      <p className="mt-1 text-[11px] text-[#8a5a32]">{attachment.error}</p>
                    ) : null}
                  </div>
                ))}
              </div>
            ) : (
              <p className="mt-2 text-[11px] text-[#6d7f93]">
          ...
          <div
            className={[
              ...
              isUser
                ? "rounded-[14px] border border-[#dce8f5] bg-[#eef3f8] px-3.5 py-2.5"
                : "pt-1"
            ].join(" ")}
          >
        ...
        {attachments.length ? (
          <div className={isUser ? "mt-1.5 flex flex-wrap justify-end gap-1.5" : "mt-1.5 flex flex-wrap gap-1.5"}>
            {attachments.map((attachment) => (
              <span
                key={attachment.id}
                className="inline-flex items-center rounded-md border border-[#d8e4f1] bg-[#f4f8fc] px-2 py-0.5 text-[11px] text-[#425a72]"
              >
                {attachment.name}
              </span>
            ))}
        ...
        >
          <Button
            ...
            variant="ghost"
            size="sm"
            className="h-7 gap-1 px-2 text-xs text-[#5b6b7d]"
            onClick={() => {
              void copyMessage();
            }}
          >
    ...
    return (
      <section className="mx-auto flex h-full w-full max-w-[980px] flex-col px-7 pb-5 pt-7">
        <div className="flex flex-1 flex-col items-center justify-center gap-3 pb-16 text-center">
          <div className="mx-auto grid h-[54px] w-[54px] place-content-center rounded-[20px] border border-[#dce4ed] bg-[#f8fbff] font-mono text-sm text-[#3c4f64]">
            {"{}"}
          </div>
          <h2 className="text-6xl font-semibold leading-none tracking-tight text-foreground">
            Let&apos;s build
        ...
        <div className="mb-3 text-right text-[34px] text-[#7e8792]">Explore more</div>
        <div className="grid gap-3 md:grid-cols-3">
          {suggestionPrompts.map((prompt, index) => (
            <Button
              ...
              type="button"
              variant="outline"
              className="h-auto min-h-[114px] justify-start whitespace-normal rounded-[22px] border-[#dfe5ec] bg-[#f8fafc] px-4 py-4 text-left text-base font-normal text-[#2b3949] hover:bg-[#f3f7fb]"
              onClick={() => onSelectSuggestion(prompt)}
            >
              <span className="mr-2 inline-grid h-6 w-6 place-content-center rounded-md border border-[#d5dde6] bg-white text-xs text-[#5d6a79]">
