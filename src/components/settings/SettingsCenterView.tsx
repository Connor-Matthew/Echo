...
const parseMcpJson = (raw: unknown): UserMcpServer[] => {
  ...
  for (const [name, value] of Object.entries(map)) {
    ...
    if (typeof entry["url"] === "string" && entry["url"]) {
      ...
      transportType =
        (entry["transportType"] as "stdio" | "streamable_http") ?? "stdio";
    }
...
const McpServerList = ({
  ...
  const handleImport = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        const fresh = imported.filter((s) => !existingNames.has(s.name));
  ...
  const commitForm = () => {
    ...
    const next: UserMcpServer = { id: form.id, name, transportType: form.transportType, endpoint, enabled: form.enabled };
    if (isAdding) {
      onChange([...servers, next]);
    } else {
      onChange(servers.map((s) => (s.id === form.id ? next : s)));
    }
    cancelForm();
  };
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    transportType: e.target.value as "stdio" | "streamable_http"
                  }))
                }
